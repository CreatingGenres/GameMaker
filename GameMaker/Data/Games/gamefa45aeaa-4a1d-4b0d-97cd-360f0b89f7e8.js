...
unitModules['speechDisplayer'] = function() { raiseEvent(this, 'speechDisplayer', arguments[0]); 
        return {
          draw: function() {
            context.save();
              ...
              for (var i in this.words) {
                context.fillText(this.words[i], 20 + dialogX + cLen, 40 + dialogY + mult * 25);
        
                cLen += context.measureText(this.words[i] + " ").width;
                if (cLen > 130) {
                  cLen = 0;
                  mult++;
                }
...
var module = createModule('collider', (function() { var data = [];  return data; })());
	...
	return RectangleUnit;
			...
			var unit = createUnit(InanimatedSpriteUnit, false, 'Basic208', 'images/library/blue-monster.png', 85.05747, 116.0763, 40, 40, 1);
			unit.dx = 0; unit.dy = 0
$.extend(unit, (function () {
		var moduleData = [];
		moduleData['move'] = (function() { var data = [];  return data; })();
moduleData['speechDisplayer'] = (function() { var data = [];  return data; })();

		return {
				
		move : unitModules['move'], 
	
		speechDisplayer : unitModules['speechDisplayer'], 
        ...
        var update = function () {
					...
					if (keyboard[68] && !previousKeyboard[68]) {
						var selected = selector('#Basic208');
						for (var i in selected) {
							try {
                                selected[i].speechDisplayer(new Object({text: 'Pesho', updateCounter: '1000', }));
                            }
                            catch(error) { 
