...
var module = createModule('collider', (function() { var data = [];  return data; })());
	...
	return RectangleUnit;
			...
			var unit = createUnit(InanimatedSpriteUnit, false, 'Basic121', 'images/library/blue-monster.png', 232.183908045977, 135.694822888283, 40, 40, 1);
			unit.dx = 0; unit.dy = 0
raiseEvent(unit, 'onSpawn');
        ...
        var update = function () {
					...
					if (keyboard[68]) {
						var selected = selector('#Basic121');
						for (var i in selected) {
							try {
                                selected[i].move(new Object({dx: '1', dy: '0', }));
                            }
                            catch(error) { 
