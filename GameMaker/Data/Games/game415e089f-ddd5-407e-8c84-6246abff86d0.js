...
$(document).ready(function () {
    game = (function () {
		...
		background.src = 'http://hugoware.co/resource/images/misc/game-background.jpg';
        ...
        var basicEventHandler = function (unit, properties, action) {
            ...
            if (action) {
            	...
            	for (var i = 0; i < selected.length; i++) {
            		selected[i][action.method].call(selected[i], action.args);
            	}
            }
        }
...
var module = createModule('collider', (function() { var data = [];  return data; })());
	...
	return RectangleUnit;
			...
			var unit = createUnit(InanimatedSpriteUnit, false, 'Basic7', 'http://static.giantbomb.com/uploads/original/0/26/9780-itsahim.JPG', 627.5862, 366.2125, 100, 100, 1);
			unit.dx = 0; unit.dy = 0
$.extend(unit, (function () {
        ...
        var update = function () {
					...
					if (keyboard[65] && !previousKeyboard[65]) {
						var selected = selector('#Basic7');
						for (var i in selected) {
							try {
                                selected[i].();
					...
					if (keyboard[83] && !previousKeyboard[83]) {
						var selected = selector('#Basic7');
						for (var i in selected) {
							try {
                                selected[i].();
